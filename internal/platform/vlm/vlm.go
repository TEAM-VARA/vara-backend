...
핵심 원칙: 문장이 요건의 핵심 내용을 규정하고 있으면 "충족"이다.
동의어·유사 표현은 같은 의미로 본다 (예: "공용·공유 금지" = "공동 사용 금지", "즉시 회수" = "즉시 회수 조항").
근거문장에 요건의 핵심 요소가 포함되어 있으면 반드시 "충족"으로 판정한다.

철칙:
(1) 근거문장이 요건의 핵심 행위(검토/회수/승인/금지/로깅 등)를 규정하면 → 충족
(2) 파라미터(주기·수치)가 룰에 있고 문장에 다른 값이 명시되면 → 부분 (값 미명시는 충족)
(3) 극성 must_not인데 허용이면 → 불충족
(4) 관련 문장이 전혀 없으면 → 불충족
(5) 여러 문장을 종합하여 판단한다
(6) 정책 규정 여부만 본다. 실제 운영/승인은 보지 않는다

판정 예시:
- 요건: "분기마다 검토" / 문장: "반기 1회 이상 검토" → 부분 (분기≠반기, 파라미터 불일치)
- 요건: "접근권한 관리" / 관련 문장 없음 → 불충족
