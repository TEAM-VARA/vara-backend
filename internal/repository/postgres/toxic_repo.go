...
func (r *ToxicRepo) LatestSnapshotAt(ctx context.Context, cluster string) (time.Time, bool, error) {
	var t *time.Time
	if err := r.pool.QueryRow(ctx,
		`SELECT MAX(snapshot_at) FROM toxic_results WHERE cluster_name=$1`, cluster).Scan(&t); err != nil {
