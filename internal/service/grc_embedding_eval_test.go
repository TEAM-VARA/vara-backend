...
func TestRuleEmbeddingThreshold_Default(t *testing.T) {
	rule := Rule{JudgmentLogic: JudgmentLogic{}}
...
func TestRuleEmbeddingThreshold_Custom(t *testing.T) {
	rule := Rule{JudgmentLogic: JudgmentLogic{SimilarityThreshold: 0.85}}
	th := ruleEmbeddingThreshold(rule)
	if th != 0.85 {
...
func TestRuleEmbeddingThreshold_InvalidZero(t *testing.T) {
	rule := Rule{JudgmentLogic: JudgmentLogic{SimilarityThreshold: 0}}
	th := ruleEmbeddingThreshold(rule)
	if th != 0.68 {
...
func TestRuleEmbeddingThreshold_InvalidOver1(t *testing.T) {
	rule := Rule{JudgmentLogic: JudgmentLogic{SimilarityThreshold: 1.5}}
	th := ruleEmbeddingThreshold(rule)
	if th != 0.68 {
...
func TestRuleEmbeddingThreshold_ExactlyOne(t *testing.T) {
	rule := Rule{JudgmentLogic: JudgmentLogic{SimilarityThreshold: 1.0}}
	th := ruleEmbeddingThreshold(rule)
	if th != 1.0 {
...
func TestBuildGuidelineText_MinimalRule(t *testing.T) {
	rule := Rule{Keywords: []string{"변경주기"}}
	text := buildGuidelineText(rule)
	if !containsSubstring(text, "식별키워드: 변경주기") {
		t.Errorf("got: %s", text)
	}
}
