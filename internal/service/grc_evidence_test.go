...
func TestEvidence_FullPipeline_StructuredRules(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.ruleID+"_"+dir, func(t *testing.T) {
			...
			result := svc.evaluateRule(context.Background(), *rule, []any{data}, []string{tc.file}, nil, nil)
...
func TestComplianceReport(t *testing.T) {
	...
	for _, tc := range cases {
		...
		evalResult := svc.evaluateRule(context.Background(), *rule, []any{data}, []string{tc.file}, nil, nil)
	...
	for _, r := range results {
		...
		evalResult := svc.evaluateRule(context.Background(), *rule, []any{data}, []string{r.tc.file}, nil, nil)
