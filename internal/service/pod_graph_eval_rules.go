...
// R-2.10.9-POD-01: 승인된 컨테이너 레지스트리 화이트리스트
//
// ISMS-P 2.10.9 악성코드 통제 — 신뢰할 수 없는 출처의 컨테이너 이미지는
// 악성코드·공급망 공격의 주요 유입 경로다. 승인된 레지스트리(approved_registries)
// 외부에서 가져온 이미지를 사용하는 Pod를 탐지한다. CIS EKS 5.1.4(승인 레지스트리)
// /5.1.3 대응. 허용 목록은 룰셋 JSON의 approved_registries로 주입(데이터드리븐).
func evalApprovedRegistry(rule Rule, req PodGraphRequest, base PodRuleResult) PodRuleResult {
	base.Severity = "high"
	podName := jsonStr(req.Pod, "metadata", "name")
	podNS := jsonStr(req.Pod, "metadata", "namespace")

	// 화이트리스트 미설정 시 평가 불가 — 룰셋에 approved_registries 필요
	if len(rule.ApprovedRegistries) == 0 {
		base.Verdict = "skip"
		base.SkipReason = "approved_registries 미설정 — 룰셋에 승인 레지스트리 목록을 지정해야 점검 가능"
		return base
	}
	...
	// initContainers + containers 모두 점검
	var allContainers []any
	allContainers = append(allContainers, jsonSlice(req.Pod, "spec", "containers")...)
	allContainers = append(allContainers, jsonSlice(req.Pod, "spec", "initContainers")...)
	...
	for _, c := range allContainers {
		...
		reg := imageRegistry(image)
		if registryAllowed(reg, rule.ApprovedRegistries) {
			matched = append(matched, fmt.Sprintf("컨테이너 '%s': 승인 레지스트리 '%s'", cName, reg))
			continue
		}
		violations = append(violations, grc.Violation{
			Field:       "containers[].image",
			Expected:    "approved registry",
			Actual:      reg,
			Description: fmt.Sprintf("Pod '%s' 컨테이너 '%s' 이미지가 미승인 레지스트리 '%s' 출처 (%s) — 신뢰 못 할 이미지 출처", podName, cName, reg, image),
			Severity:    "high",
			K8sSource:   grc.K8sSource{Namespace: podNS, ResourceKind: "Pod", ResourceName: podName, ContainerName: cName},
		})
	}
	...
	if len(violations) > 0 {
		base.Verdict = "미준수"
		base.Violations = violations
		base.Remediation = "승인된 레지스트리(approved_registries)의 이미지만 사용하도록 변경하고, Admission(Kyverno/Gatekeeper)으로 허용 레지스트리를 강제하세요. 정당한 예외는 malware-exception/justification annotation으로 관리."
	} else {
		base.Verdict = "준수"
		base.MatchedIndicators = matched
	}
...
// imageRegistry는 컨테이너 이미지 참조에서 레지스트리 호스트를 추출한다.
// 예) "123.dkr.ecr.ap-northeast-2.amazonaws.com/app:1.0" → "123.dkr.ecr.ap-northeast-2.amazonaws.com"
//
//	"nginx:1.25" → "docker.io",  "bitnami/nginx" → "docker.io"
func imageRegistry(image string) string {
	s := image
	if i := strings.Index(s, "@"); i >= 0 { // digest 제거
		s = s[:i]
	}
	slash := strings.Index(s, "/")
	if slash < 0 {
		return "docker.io" // 단일 세그먼트 = Docker Hub 공식 이미지
	}
	first := s[:slash]
	// 첫 세그먼트에 '.'(도메인) 또는 ':'(포트)가 있거나 localhost면 레지스트리 호스트
	if strings.ContainsAny(first, ".:") || first == "localhost" {
		return first
	}
	return "docker.io" // "library/nginx" 같은 Docker Hub 네임스페이스
}

// registryAllowed는 호스트가 승인 목록에 포함되는지 판정한다.
// "*." 접두 항목은 서브도메인 suffix 매칭(예: "*.amazonaws.com" → "...amazonaws.com").
func registryAllowed(host string, approved []string) bool {
	for _, a := range approved {
		...
		if strings.HasPrefix(a, "*.") {
			if host == a[2:] || strings.HasSuffix(host, a[1:]) {
				return true
			}
		} else if host == a {
			return true
